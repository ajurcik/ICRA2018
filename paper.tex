...
This can be formulated as a motion planning problem in a high-dimensional configuration space and solved using sampling-based planners.
...
The performance of PRM-based planners can be improved  by increasing probablity of sampling in regions close to obstacles. 
...
growth of the configuration tree may be toward the random samples may be blocked by an obstacle~\cite{vonasekphd}.
Instead, the random samples has to be generated in the dififcult regions only if the tree can approach them.
...
In this paper, we propose a novel modification of the RRT planner for computing trajectories insider protein tunnels.
To cope with the narrow passage problem, the random samples are generated mainly around a tunnel computed using Voronoi diagrams and 
not in the whole conifguration space as in~\cite{cortes2010simulating}.
To further increase the probability of sampling inside the narrow passage, the ligand atoms are shrunk, which is similar
to~\cite{cortes2010simulating}.
The ligand flexibility is modeled using a predefied set of typical conformations, therefore the number of dimensions of the configuration space does not depend on the DOF of the ligand as in~\cite{cortes2010simulating}.
