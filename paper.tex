...
For each conformation $\l \in \L$ of the ligand, a KD-tree is build for the ligand at configuration $q=(0,0,0,0,0,0,l,\smax)$
(i.e., for ligand with no translation and no rotation).
To find a nearest atom to $\qrand$ for a ligand at the configuration $q=(x,y,z,r_x,r_y,r_z,l,s)$, the translation vector $T=(x,y,z)$ and
...
Two motion planners were utilized: the proposed planner (Alg.~\ref{alg::main} and Alg.~\ref{alg::expand}), denoted as \RA\ in the rest of the paperm, and the Retraction-based RRT~\cite{zhangRetraction} denoted as \RB\ in the rest of the paper.
The \RA\ method was run with the following parameters:
$\Imax=10,000$, $m=100$, $\gb=0.9$, $\dt=1.5$\AA, $\rv=2$~\AA.
...
The Retraction-based RRT was implemented using 6D Euclidean metric.
