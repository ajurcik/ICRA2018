...
The experiments show the advantage of the proposed motion planner. 
The planner outperforms the Retraction-based planner not only in the `easy' problems (with $\smin=0.5$), but also
in the case of difficult scenarios with larger ligands ($\smin=0.8$).

It is worth to mention, that the performance of both planners is influenced by their parameters, namely by
$\Imax$.
By increasing this parameter, we can expect better performance in finding the trajectories and consequently higher 
traversability rates, but with the increased computational time.

The internal performance of the planners is however not the main criteria.
Fom bio-chemical point of view, the main contribution of using motion planning for detecting the ligand trajectories is the ability to
consider 
