...
In order to determine if the ligand can pass the tunnel, it is necessary to compute a trajectory considering the shape of the ligand and its 3D translation and rotation.
...
To cope with these many-DOF ligands, the RRT-ML~\cite{cortes2007mlrrt} approach is employed as the basic planner.
...
The main loop of the planner (Alg.~\ref{alg::main}) is similar to the original RRT, but we propose its three modifications.
...
The proposed approach was used to analyze tunnel traversability towards the active site of the Haloalkane dehalogenase protein (PDB ID 1CQW).
