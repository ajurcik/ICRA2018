...
\begin{abstract}
Sampling-based motion planners can compute trajectories for many-DOF (Degrees Of Freedom) robots (objects) of various shapes, which finds
applicatoins even beyond robotics, e.g. for studing accesible pathways of proteins in computational bio-chemistry.
Chemical interactions of proteins and other molecules take place in actives sites that are usually deeply burid inside the proteins.
...
The Voronoi-based tunnel computation is however not suitable for ligands of arbitrary shapes.
In this paper, we propose to analyze the accessibility of the active sites using motion planning.
The require to cope with the narrow passage problem, as the proteins are dense structures which limits possible movements of the ligands.
To cope with the narrow passage issue, we propose to guide the search in the corresponding configuration space using the Voronoi-based tunnels.
We propose to imitate the ligands flexibility by utilizing a library of known (typical) conformations, which significantly decreases 
the dimension of the configuration space.
We propose to measure distances between the configurations using a metric that considers also the shape of the ligand.
Old:
Motion planning can be used to analyze accessibility of tunnels by small molecules (ligands), which can help chemists to ..
The ligands are not rigid structures, but they are rather flexible.
In this paper, we present sampling-based motion planinng method to compute trajectories of the ligands inside know protein tunnels.
The tunnels are represented by a set of spheres and they are used to focus sampling of the configuration space.
To decrease the number of degrees of freedom, the ligand flexibility is modeled by a set of predefined conformations.
We propose a new metric to measrue distances between the configurations which considers the shape of the ligands.
\end{abstract}
...
As the random samples are classify as free or non-free using collision detection, the sampling-based methods can found paths for roobts of various shapes. 
...
The well known issues is the narrow passage problem and the sensitiviy to distance metric in the configuration space.

A narrow passage is a small region in the configuration space whose removal changes connectivity of the space.
As the volume of the narrow passage is significantly smaller than then volume of the whole configuration space, it is difficult to 
sample it dense enough using uniform ditribution.
Consequently, many iterations are needed in order to put enough samples into it, which increases the computation time.
...
%Besides robotics, sampling-based planners have been used also in applications in bio-i
...
Sampling-based motion planning methods are suitable candidates for this task, as they can consider non-spherical shape of the ligand as well as the conformational changes.
...
The dilatation of the ligand free space can be achieved by shrinking the atom radii, similarly, e.g., to~\cite{cortes2005path}.
