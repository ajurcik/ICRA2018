...
Based on the observation from MD simulations, the tunnels adapt to the ligands (and vice versa), so even narrow tunnels can serve as transportation path for thhe ligands TODO CITACE??.
...
The reason is, that \RA\ expands the trees more closer to the random samples $\qrand$ than \RB.
This results in faster exploration of the conifguration space and therefore, \RA\ needs less number of iterations to reach the goal
configuration.
