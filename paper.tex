...
However, in protein dynamics with ligand, the tunnels can move, merge and adapt to the shape of the passing ligand and vice versa.
...
MD simulations of ligands passing through proteins further show that the ligands can be highly flexible.
The possible conformations of a ligand are determined by the torsion and dihedral angles between its atoms. 
The low-energy conformations are preferred.
...
The proposed algorithm was verified in two scenarios: on a sequence of frames of MD simulation and on a set of protein conformations provided 
the conformation sampling approach.
...
Most of the used 50 ligand conformations where used in the planning, which is shown in Tab.~\ref{tab::m040c}.
...
As the tested ligand is a known inhibitor of the protein 4L2L~\cite{marques2017enzyme} we assume that the ligand can reach the active site.
Due to large size of the ligand (33 atoms), low scale-down factors $\smin\in\{0.4,0.5,0.6\}$ were used.
The traversability detected by the \RA\ method is show in Tab.~\ref{tab::4l2l} and the example
of a final trajectory to the active site is depicted in Fig.~\ref{fig::inhibitor}.
