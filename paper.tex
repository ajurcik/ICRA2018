...
The random samples are not generated from the whole configuration space, but the sampling is guided using
the spheres of the tunnel.
This allows us to find trajectories along the given tunnel, which is faster than searching any trajectory in the whole protein.
...
The ligand flexibility is modelled using a fixed set of predefined conformations.
...
This requires the analysis of contact space, which leads to the generalized penetration depth computation of high complexity~\cite{he2016efficient}. %\cite{he2016efficient} in RAL!! + zhang2008fast
...
Traditionally, a line is constructed from $\qnear$ to $\qrand$ and the tree is expanded by a configuration located on this line.
...
To evaluate $\da(q_1,q_2)$, the absolute positions of atoms of the ligand defined by $q_1$ and $q_2$ are computed.
...
By employing this metric in the expansion procedure, the tree is expanded by such a new configurations, whose atoms maximally approach
the atoms of the configuration $\qrand$.
We observed that this leads to a better construction of the configuration tree inside the proteins and it increases probability
...
The atom-based metric can be evaluated fast with complexity $O(\log n)$, where $n$ is the number of atoms of the ligand, using KD-trees.
...
Then, $\qrand$ is translated to $\qrand' = R^{-1}(\qrand-T)$ and the KD-tree search is used to find the nearest point towards $\qrand'$, which
...
In most of the frames, two tunnels were detected.~\footnote{%
We refer to~\url{http://mrs.felk.cvut.cz/icra2018protein} for additional images.
}
...
Visualization of trajectories for the ligand DCP   in the two examined tunnels.  
    ...
    For the purpose of this image, also scales $\smin=0.3$ and $\smin=0.4$ where tested.
}
\end{figure}
