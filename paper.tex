...
pathway detection~\cite{cortes2010simulating,cortes2005path}, for tunnel detection~\cite{vonasek2017tunnel,vonasek2016application}, and especially for protein folding~\cite{raveh2009rapid,amato2002using}. %novinskaya2015improving
...
The RRT-based planners cannot cope with the narrow passage problem simply by increasing the probability of sampling in the difficult regions, as the growth of the configuration tree towards the random samples may be blocked by obstacles. %~\cite{vonasekphd}.
...
The input of the proposed method is a set of tunnels computed using Voronoi diagrams, and the task is to find trajectories for a ligand along these tunnels leading from outside the protein to the active site.
The tunnels are detected on selected protein conformations resulting e.g. from MD simulations or from tool providing conformation sampling
...
by the 3D position $(x,y,z)$ of the reference point of the ligand (e.g. the geometric center or a selected atom), the rotation around $x$, $y$, and $z$ axes,
...
In the case of alone tunnels (which is very common), we recommend to use double value of the tunnel average width.
...
The proposed algorithm was verified in two scenarios: on a sequence frames of MD simulation and on a set of protein conformations provided by the tCONCOORD tool~\cite{seeliger2007geometry}.
...
The tunnels were detected in 100 consecutive frames of MD simulation in part where the protein was stabilized.
The tunnels for probe 0.9~\AA\ towards the active site defined by residues 38 (Asparagine), 102 (Histidine), and 103 (Aspartic Acid) were
...
The number of generated ligand conformations $|\L|$ depends on the number of rotatable bonds of the ligands and eventhough only
low-enery conformations are stored in $\L$, the set can still contain thousands of conformations.
For a fast motion planning, we utilize a subset of the original set $\L$.
The aim of this experiment was to verify the procedure to select the subset. 
...
The task of the second analysis was to find trajectory for the ligand 4-(4-benzylphenyl)-1,3-thiazol-2-amine into the active
site of A4 hydrolase (PDB ID 4L2L).
...
The tCONCOORD tool~\cite{seeliger2007geometry} was used to generate 1000 conformations of the protein 4L2L (without the inhibitor).
The initial pdb structure was minimized by GROMACS 4.0.7~\cite{hess2008gromacs} using the input parameters recommended on the example page for tCONCOORD. 
The protein conformations were generated using default settings for tCONCOORD in the PERT mode. % which only perturbs the starting configuration of atoms instead of complete randomization.
The tunnels to the active site defined by residues 134 (Alanine) and 311 (Phenylalanine) were computed by CAVER 3.0 for the probe 0.9~\AA.
All 1000 conformations contained at least 5 tunnels, more than 900 conformations contained 9 tunnels.
The detected the tunnels slightly differ due to conformational changes.
Therefore, the traversability towards the active site was computed for the first three tunnels (with average bottlenecks 1.22~\AA, 1.20~\AA\ and 1.19~\AA\ with the average lengths 22.3~\AA, 25.3~\AA\ and 28.4~\AA, respectively).
The conformations of the ligand were prepared in the same way as in the previous section.
From the available $|\L|=736$ conformations, 100 were selected using the `S' strategy.


As the tested ligand is a known inhibitor of the protein 4L2L, we assume that the ligand can reach the active site.
We tested several scaling-down factors from $\smin=0.4$ to $\smin=0.6$.

Example of a found trajectory with the depicted final position of the ligand is depicted in Fig.~\ref{fig::inhibitor}.
TODO: report num of travewrsable frames
...
\begin{figure}
\centering
\includegraphics[width=0.4\textwidth]{fig/4l2l-detail}
\caption{\label{fig::inhibitor}Example of inhibitor in 4L2L ... TOOO some better image }
\end{figure}
...
{\color{red}
    Neni to blbost? (ta prvni veta)
Determination of the proper scaling-down factor is beyond the scope of this paper, where we focus on the algorithmic part of the problem.
The computed trajectories are further visualized and analyzed.
The scales used at the trajectory points brings the biochemists new knowledge about the tunnels, which was impossible to get using the VD-based computations.
...
shown also in the second experiment, where the trajectory for inhibitor of the 4L2L protein required scaled-down to 0.5 or 0.6.
