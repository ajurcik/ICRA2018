...
Therefore, computational approaches that consider only the static geometry of the ligand are still preferable due to their high speed.
...
Computationally less demanding approaches for generating protein conformations are based on constrained geometric simulations, such as
...
The tunnels are detected on selected protein conformations, resulting, e.g., from MD simulations or from a tool providing the conformation sampling
...
However, the tunnels can adapt their shape to the passing ligands and vice versa.
...
if both $q_1$ and $q_2$ are collision-free and if the distance from $q_1$ to $q_2$ is smaller than the resolution of the planner ($\varepsilon=0.05$~\AA).
...
In case of tunnels not having other tunnels in the close vicinity (which is very common), we recommend to use the double value of the tunnel average width.
...
The random samples are generated similarly as in case of $\qrand$ samples, the translation part $(x,y,z)$ is generated around $\qnear$.
...
Low deviation is characteristic for the packed conformations, while larger deviations correspond to the elongated conformations.
...
The aim of this experiment was to verify the procedure for selection of the subset. 
...
than using the smaller number of conformations or when preferring packed conformations.
...
\caption{\label{tab::selection}
    ...
    The tested protein consists of 4650 atoms. 
...
Besides scaling-down the ligand, also the atoms of the protein can be scaled, which is used, e.g., in~\cite{cortes2005path}.
The tool~\cite{cortes2005path} uses the scale 0.8 for both the ligand and protein.
...
The detected the tunnels slightly differ due to the conformational changes.
Therefore, the ligand traversability towards the active site was computed for the first three tunnels (with average bottlenecks 1.22~\AA, 1.20~\AA\, and 1.19~\AA\ with the average lengths 22.3~\AA, 25.3~\AA\, and 28.4~\AA, respectively).
...
The example of the found trajectory showing the final position of the ligand is depicted in Fig.~\ref{fig::inhibitor}.
TODO: report num of traversable frames
...
\caption{\label{fig::inhibitor}Example of inhibitor in 4L2L ... TODO some better image }
\end{figure}
...
According to the results of the motion planing, the DCP ligand needs to shrink down to approximately 75~\% of its size, but only at one part of the tunnel (Fig.~\ref{fig::dcp}, point B). 
