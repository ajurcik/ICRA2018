...
The knowledge about a tunnel (e.g. the length, bottleneck or physico-chemical properties) can help chemists to estimate possible interactions
between the protein and the ligand.
Voronoi-diagrams are usually used to detect the tunnels, but this is useful only for estimate traversability of small spherical ligands.
Computing motions of non-spherical ligands including their possible conformation changes (i.e., when atoms of the ligand move relatively to each other) leads to motion planning in high-dimensional configuration space, which can be solved using sampling-based planning methods.
Such a planning problem necessarily includes narrow passages, as the movements of ligands are limited by many protein atoms.
Moreover, considering flexible ligands increase the degrees of freedom, and consequently the dimension of the configuration space.
%Moreover, as the flexible ligands have various shapes, the selection of a proper metric in the configuration space is crucial.
...
The flexibility of the ligand is modeled using a fixed set of known conformations, so the dimension of the configuration space is not increased.
The proposed method has been evaluated in a virtual screening scenario.
\end{abstract}
...
The sampling-based planners is that they can cope with robots with many degrees of freedom (DOF).
...
Besides many applications in robotics, sampling-based planners have been used e.g. in bio-chemistry 
...
or for protein folding~\cite{raveh2009rapid,novinskaya2015improving}.
...
leading to a specific place (called active site) inside the protein (MAME CITACI?)
...
Such a  tunnel has to be wide enough and the physico-chemical properties of the surrounding amino acids have to be compatible with the ligand (Fig.~\ref{fig::motiv}).
...
This motion planning however requires to cope with the narrow passage problem, as the proteins are dense structures and the
...
Voronoi diagrams~\cite{yaffe2008,caver3} or grid-based methods~\cite{sehnal2013mole} are often used for tunnel computation.
...
Due to its low volume, the probability of sampling the narrow passages is low.
...
The generalization of~\cite{kardossRRTKK} is the guided sampling, where a whole path (computed e.g. in the workspace) is used
to generate samples in the configuration space~\cite{vonasek2009rrt,denny2016dynamic,denny2014marrt}.
...
In Retraction-RRT~\cite{zhangRetraction}, the random sample $\qrand$ is connected to the tree if the line segment 
from $\qnear$ to $\qrand$ is collision-free.
Otherwise, the retraction-step is performed.
...
of the generalized penetration depth, which has high complexity~\cite{zhang2008fast,he2016efficient}. %\cite{he2016efficient} in RAL!!
...
In contrast to~\cite{cortes2010simulating}, which find any exit pathway, we propose to compute the trajectories only around/inside each tunnel
separately.
...
The main loop of planner, which is described in Alg.~\ref{alg::main} is similar to the original RRT planner.
...
The second modification to the RRT is the novel expansion procedure (Alg.~\ref{alg::expand}), which generates new collision-free nodes around $\qnear\in\T$.
...
First, the maximal scale $\smax \in \S$ is tested and $m$ random samples are generated around $\qrand$ and tested for collision.
...
procedure utilize a metric that is sensitive to the shape of the ligands.
...
$q$ being tested.
This metric is denoted ($\da(\cdot,\cdot)$ on line~\ref{alg::expand:a} in Alg.~\ref{alg::expand}).
...
The `longer' conformations can better fit in a narrow passage, but their rotation inside the narrow passage is problematic.
On the other hand, the `packed' conformations cannot fit the narrow passage, but they can more likely rotate.

In the third experiment, we tested how the selection of conformations influences the ability of the planners to find a solution.
Two strategies were used to select the conformations.
In the first strategy, denoted as Packed, $n$ `packed' conformations are selected from the available set of conformations.
The second strategy, denoted as Various, selects $n$ conformations as follows.
...
The conformations are sorted according to this deviation and then $n$ conformations are selected from this sorted list uniformly (i.e., the
        conformation number $1$, then $1+n$, etc.).
