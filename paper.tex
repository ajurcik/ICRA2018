...
This necessary requires to consider also conformational changes.
...
solve by sampling-based planners.
...
Another advantage of our method in comparison with the existing approaches is that we enable the users to trace the ligand path leading from the outside solvent towards the protein active site.

Our proposed method was tested by the biochemists who compared the results with already known trajectories of ligands, determined by their traditional approaches.
In the following we first mention several approaches and methods related to our solution and then describe our proposed method in detail.
...
Movements of the ligands inside tunnels are blocked by many atoms forming the tunnel.
...
distances then characterize the spread.
...
MD of the protein was computed using the AMBER~12 (details about the MD simulations of DhaAwt/4E46 are described in~\cite{marques2017catalytic}).
...
The performance of \RA\ and \RB\ planners is first measured by finding trajectories for only one atom of VDW radii $1.7$~\AA.
...
The results are same as for ligand scale $\smin=0.6$ (except DCP and m040 which can be result of the randomization).
...
The comparison between \RA\ and \RC\ shows that utilization of the  atom-based metric (used in \RA) leads to higher traversability rate
...
to distance $2$~\AA or less and they were considered unsuccessful otherwise.
